...
st.markdown("""
    ...
    .stButton>button { border-radius: 4px; height: 25px; line-height: 10px; font-size: 0.7rem; margin-top: 5px; }
    </style>
...
def save_task(name, cat, dt, notes, task_id=None):
    # Time ko ISO format mein save karne se pehle IST ensure karein
    ...
    with st.expander("➕ " + ("Edit Task" if is_edit else "Add New Entry"), expanded=is_edit):
        with st.form("input_form"):
            ...
            if st.form_submit_button("Save"):
                # Input time ko IST ke saath jodein
    ...
    for i, (cat_name, cat_color) in enumerate(cats):
        with dashboard_cols[i]:
            ...
            for _, row in active.iterrows():
                # Yahan comparison ab dono IST mein hain
